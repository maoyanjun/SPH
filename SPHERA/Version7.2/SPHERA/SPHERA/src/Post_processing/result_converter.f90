...
character(len=120) :: filetec !sunjw
...
' Warning: nblocchi>maxnumblock in subroutine result_converter.'
 ...
 ! write(*,*)cargo,cargo(1:len_trim(cargo))
   open(unit=unittec,file=filetec,form='formatted',access='sequential',        &
      status='unknown')

   rewind(unit=unittec)
         write(unittec,'(a)') ' TITLE= "tecplot results" '       
         write(unittec,'(a)') '  VARIABLES = "X"  "Y"  "Z" "U"  "V" "W" "pres" "dens" "mass"  "visc"  '     
         write(unittec,'(a)') ' ZONE '  
         write(unittec,'(A30,1X,A6)') 'STRANDID=1, SOLUTIONTIME=',cargo
         write(unittec,'(A2,1X,I6,1X,A20)') 'I=',numpoints,', J=1, K=1, F=POINT  '   
 !        write(unittec,'(a)')        
!200   format(1X, A4,1X,I6)
...
!---------------------------------------------------------sunjw   
    do i=1, numpoints
        write (unittec,'(10(1x,F10.3))') pg(finger(i))%coord(1),pg(finger(i))%coord(2),pg(finger(i))%coord(3),&
                                                        pg(finger(i))%vel(1), pg(finger(i))%vel(2),pg(finger(i))%vel(3),&
                                                        pg(finger(i))%pres,pg(finger(i))%dens,pg(finger(i))%mass,pg(finger(i))%visc
    enddo
!---------------------------------------------------------sunjw      
