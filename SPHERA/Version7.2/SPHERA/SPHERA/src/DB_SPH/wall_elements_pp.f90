...
!///////////////////定义点压力输出文件
   ...
   write(nomefilectl_wall,"(a,a,i8.8,a)") nomecaso(1:len_trim(nomecaso)),".NBpress"      

   open (unit_dbsph_se_ID,file=nomefilectl_wall,status="unknown",access="append")      
...
!         pres_wpp(DBSPH%monitor_IDs(i)),pg_w(DBSPH%monitor_IDs(i))%wet  
    write(unit_dbsph_se_ID,'(F10.3,1X,100(1x,F10.3))') tempo, pg_w(DBSPH%monitor_IDs(i))%pres
 !  end do   
