...
! Description: Diagnostic (error) messages.             Õï¶Ï´íÎóÐÅÏ¢   
!----------------------------------------------------------------------------------------------------------------------------------
!call diagnostic(arg1=1)sunjw 
...
if (present(avalue)) then
   stringa = adjustl(avalue) !½«avalue×ó±ßµÄ¿Õ°××Ö·û·Åµ½ÓÒ±ß¸³Öµ¸østringa  
   la = len_trim(stringa)!È¥µôstringaÓÒ²à¿Õ°××Ö·ûÖ®ºóµÄ³¤¶È
   else
      stringa = ' '
      la = 0
   endif
...
select case (ierr)
   case (1)    
! Errors in arguments  ÆÁÄ»Êä³ö´íÎóÐÅÏ¢
      write(nscr,*) "-----------------------------------"
