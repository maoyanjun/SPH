...
do while (TRIM(lcase(ainp))/="##### end vertices #####")
   select case (TRIM(Domain%tipo))
      case ("semi","bsph") 
         ...
         !读入角点坐标
         !i:角点编号
         !values1(1:NumberEntities(1)):角点坐标
         write(label,"(i8)") i
   ...
   ! i:角点编号
   !(xyzlabel(icoordp(n,ncord-1)), Vertice(icoordp(n,ncord-1),i),n=1,ncord)
   !     1  X  0.0000E+00  Z  3.0000E-01
   !ncord:角点总数
   !xyzlabel(icoordp(n,ncord-1)):X,Y,Z
   !Vertice(icoordp(n,ncord-1),i)第i个角点的X/Y/Z 角点坐标
   call ReadRiga (ainp,comment,nrighe,ioerr,ninp)
   if (.NOT.ReadCheck (ioerr,ier,nrighe,ainp,"VERTICES DATA",ninp,nout)) return
enddo
