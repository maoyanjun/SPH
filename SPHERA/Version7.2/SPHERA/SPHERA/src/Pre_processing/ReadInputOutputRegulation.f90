...
integer(4) :: icpoi_fr_points!///add time series of points cpoi_fr_points
integer(4) :: ioutpo2,ioerr
double precision :: plot_fr,memo_fr,rest_fr,cpoi_fr,pllb_fr,depth_dt_out
double precision :: cpoi_fr_points
character(80) :: token
...
rest_fr = zero
icpoi_fr = 0
icpoi_fr_points = 0!///add time series of points
cpoi_fr = zero
cpoi_fr_points = zero
ipllb_fr = 0
ipllb_md = 0
...
!///add time series of points
      case ("controlpoints")
         ...
         if(token(1:4)=="step") then 
            token = lcase(GetToken(ainp,3,ioerr))
            if (.NOT.ReadCheck(ioerr,ier,nrighe,ainp,                          &
               "CONTROL POINTS TIME SERIES SAVING FREQUENCY STEP value",ninp,nout))        &
               return
            read(token,*,iostat=ioerr) icpoi_fr_points
            if (.NOT.ReadCheck(ioerr,ier,nrighe,ainp,                          &
               "CONTROL POINTS TIME SERIES SAVING FREQUENCY STEP value",ninp,nout))        &
               return 
            elseif (token(1:4)=="time") then 
               token = lcase(GetToken(ainp,3,ioerr))
               if (.NOT.ReadCheck(ioerr,ier,nrighe,ainp,                       &
                  "CONTROL POINTS TIME SERIES SAVING FREQUENCY TIME value",ninp,nout))     &
                  return
               read(token,*,iostat=ioerr) cpoi_fr_points
               if (.NOT.ReadCheck(ioerr,ier,nrighe,ainp,                       &
                  "CONTROL POINTS TIME SERIES SAVING FREQUENCY TIME value",ninp,nout))     &
                  return
            endif            
           
!///end  add time series of points      
...
if (ncord>0) then
   ...
   Domain%icpoi_fr_points = icpoi_fr_points!///add time series of points
   Domain%cpoi_fr = cpoi_fr
   Domain%cpoi_fr_points = cpoi_fr_points
   ...
   if (nout>0) then 
      ...
      write(nout,"(1x,a,e12.4)") "Ctrl.Points    time frequency : ",           &
         Domain%cpoi_fr
!///add time series of points      
      write(nout,"(1x,a,i12)")   "Ctrl.Points time series step frequency : ",           &
         Domain%icpoi_fr_points
      write(nout,"(1x,a,e12.4)") "Ctrl.Points time series time frequency : ",           &
        Domain%cpoi_fr_points    
