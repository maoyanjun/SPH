!----------------------------------------------------------------------------------------------------------------------------------
! 本程序基于SPHERA v.7.2版本改进版
! SPHERA v.7.2 (Smoothed Particle Hydrodynamics research software; mesh-less Computational Fluid Dynamics code).
...
do n=1,narg 
   do i=1, len(nomearg)
      nomearg(i:i) = " "   !将字符串nomearg全部定义为空白
   enddo
   call getarg(n,nomearg) !读取第N个命令行参数，放在nomearg
                          !
   if (nomecaso=="??????") then
      nomecaso = nomearg ! 
      nomefile(0) = trim(nomecaso)//".inp" !输入文件
      nomefile(1) = trim(nomecaso)//".out" !Log 文件
      nomefile(2) = trim(nomecaso)//".res"
      nomefile(3) = trim(nomecaso)//".rst"
      nomefile(4) = trim(nomecaso)//".plb" ! Free surface 
...
1x,'Roberto Guandalini, David Zuccal?.                                     '/ &
...
write(nout,*)
! Input 
call start_and_stop(2,2)   !第一个参数2，判断Gest_Input函数开始执行的时间
call Gest_Input            !第二个参数2，标记Gest_Input函数
call start_and_stop(3,2)   !第一个参数3，判断Gest_Input函数执行结束的时间
starttime = zero
! Main algorithm 
...
open(nout,file=nomefile(1),status="unknown",form="formatted",iostat=ier) !Log文件
if (ier/=0 ) call diagnostic(arg1=2,arg2=1)  !write(nout,'(1x,a)') '  1 - Output file cannot be opened.'
fileexist = .false.
! To check the input file
inquire(file=nomefile(0),exist=fileexist)
if (.not.fileexist) then
   call diagnostic(arg1=2,arg2=2) !write(nout,'(1x,a)') '  2 - Input file does not exist.'
   else
      open(ninp,file=nomefile(0),status="old",form="formatted",iostat=ier)
      if (ier/=0) call diagnostic(arg1=2,arg2=3) ! write(nout,'(1x,a)') '  3 - Input file cannot be opened.'
endif
return
