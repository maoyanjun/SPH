...
enddo
! Introductory procedure for inlet conditions  程序入口条件
...
if (vtkconv) then
   ...
   !call result_converter_tec ('inizio')  !sunjw tecplot
endif
...
! It assigns the movement with an imposed kinematics ("npointv" velocity data)
         if (Partz(ir)%npointv>1) then
            call vellaw(Partz(ir)%vlaw,Partz(ir)%vel,Partz(ir)%npointv)!!!!!!指定固定边界速度
...
! Fixed value of velocity (npointv = 1)  !速度标记
...
!//--------------------------------------------------------------- 
!//////////////////////////////////////////////////////////////////   
   if (Domain%icpoi_fr_points>0) then                                                           !sunjw
              if ((mod(it,Domain%icpoi_fr_points)==0).AND.(npointst>0)) then !sunjw
                  call memo_ctl_points                                                                     !sunjw
            endif                                                                                                     !sunjw
        elseif (Domain%cpoi_fr_points>zero) then                                             !sunjw
             if ((mod(tempo,Domain%cpoi_fr_points)<=dtvel).AND.(npointst>0)) then !sunjw
                 call memo_ctl_points                                                                       !sunjw
            endif   
  endif
...
! Paraview output 
   if (vtkconv) then
      ...
      !call result_converter_tec ('loop__')  !sunjw tecplot
   endif
...
if (vtkconv) then
   call result_converter ('fine__')
      !call result_converter_tec ('fine__')  !sunjw tecplot
endif
if (nout>0) then
   write (nout,*) " "
   write (nout,'(a)')                                                          &
